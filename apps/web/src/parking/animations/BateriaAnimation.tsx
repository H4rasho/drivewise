...
// Car center origin (0,0). rotation=0 → front pointing UP. rotation=270 → front pointing left.
...
export default function BateriaAnimation({ step }: Props) {
  ...
  return (
    <svg viewBox="0 0 320 310" className="w-full h-full" aria-hidden>
      <defs>
        <linearGradient id="roadGradB" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#1c1c1c" />
          <stop offset="100%" stopColor="#252525" />
        </linearGradient>
        <linearGradient id="carBodyGradB" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#14d489" />
          <stop offset="50%" stopColor="#10b981" />
          <stop offset="100%" stopColor="#059669" />
        </linearGradient>
        <linearGradient id="carRoofGradB" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#047857" />
          <stop offset="100%" stopColor="#065f46" />
        </linearGradient>
        <filter id="carShadowB" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="2" dy="3" stdDeviation="3" floodColor="#000" floodOpacity="0.45" />
        </filter>
      </defs>

      {/* Road (top) */}
      <rect x="0" y="0" width="320" height="140" fill="url(#roadGradB)" />

      {/* Road center dashes */}
      {Array.from({ length: 11 }).map((_, i) => (
        <rect key={i} x={i * 32 + 6} y="66" width="20" height="5" rx="1.5" fill="#ffffff" opacity="0.11" />
      ))}

      {/* Road/parking separator */}
      <line x1="0" y1="140" x2="320" y2="140" stroke="#f0c040" strokeWidth="1.5" opacity="0.4" />
      <line x1="0" y1="140" x2="320" y2="140" stroke="#ffffff" strokeWidth="0.5" opacity="0.1" />

      {/* Parking lot area */}
      <rect x="0" y="140" width="320" height="140" fill="#1e1e1e" />

      {/* Sidewalk (bottom) */}
      <rect x="0" y="280" width="320" height="30" fill="#161616" />
      {/* Sidewalk texture */}
      {Array.from({ length: 20 }).map((_, i) => (
        <rect key={i} x={8 + (i % 10) * 30} y={285 + Math.floor(i / 10) * 10} width="3" height="3" rx="1" fill="#222" opacity="0.8" />
      ))}
      <line x1="0" y1="280" x2="320" y2="280" stroke="#2e2e2e" strokeWidth="1.5" />

      {/* Parking space dividers */}
      {[0, 80, 160, 240, 320].map((x, i) => (
        <line key={i} x1={x} y1="140" x2={x} y2="280" stroke="#ffffff" strokeWidth="1.2" opacity="0.15" />
      ))}

      {/* Parking space number hints */}
      {[40, 200].map((x, i) => (
        <text key={i} x={x} y="275" textAnchor="middle" fill="#ffffff" fontSize="9" opacity="0.18" fontFamily="monospace">{i === 0 ? "01" : "03"}</text>
      ))}

      {/* Parked car — Left space (0-80) */}
      <StaticCarH x={40} y={192} />

      {/* Parked car — Right space (240-320) */}
      <StaticCarH x={280} y={192} />
...
function Car() {
  ...
  return (
    <g transform={`translate(${-hw}, ${-hh})`}>
      ...
      {/* Body */}
      <rect x="0" y="0" width={W} height={H} rx="9" fill="url(#carBodyGradB)" />
      ...
      {/* Roof */}
      <rect x="7" y="39" width={W - 14} height="24" rx="3" fill="url(#carRoofGradB)" />
...
// ─── Static parked car (gray), centered at (x,y), top-view vertical ─────────
function StaticCarH({ x, y }: { x: number; y: number }) {
  const W = 54, H = 90;
  const hw = W / 2, hh = H / 2;
  return (
